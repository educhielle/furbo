...
{
    if ( a[0].size() != b.size() ) throw "Matrix and std::vector have incompatible sizes";
...
add(const std::vector<std::vector<std::vector<std::vector<T>>>> & a, const std::vector<U> & b)
{
    ...
    std::vector<std::vector<std::vector<std::vector<T>>>> c(
        nItems, std::vector<std::vector<std::vector<T>>>(
            nRows, std::vector<std::vector<T>>(
                nCols, std::vector<T>(nChannels)
    )));
...
template <class T> std::vector<size_t>
argmax(const std::vector<std::vector<T>> & m)
{
    std::vector<size_t> v;
...
countEqual(const std::vector<T> & a, const std::vector<T> & b)
{
...
template <class T> T
mean(const std::vector<std::vector<std::vector<std::vector<T>>>> & a)
{
...
template <class T>
T product_inplace(std::vector<T> & v)
{
    auto size = v.size();
...
{
    std::vector<std::vector<std::vector<std::vector<T>>>> r(
        dimensions[0], std::vector<std::vector<std::vector<T>>>(
            dimensions[1], std::vector<std::vector<T>>(
                dimensions[2], std::vector<T>(dimensions[3]))));
    size_t i,j,k,l;
    i=j=k=l=0;
    bool full = false;
    for ( auto & v : m )
...
reshapeOrder(const std::vector<std::vector<T>> & a, const std::vector<size_t> order)
{
    ...
    std::vector<size_t> sizeA = { a.size(), a[0].size() };
    std::vector<size_t> sizeR;
    for ( auto e : o ) sizeR.push_back( sizeA[e] );
    std::vector<std::vector<T>> r( sizeR[0], std::vector<T>(sizeR[1]) );
...
reshapeOrder(const std::vector<std::vector<std::vector<T>>> & a, const std::vector<size_t> order)
{
    ...
    std::vector<size_t> sizeA = { a.size(), a[0].size(), a[0][0].size() };
    std::vector<size_t> sizeR;
    for ( auto e : o ) sizeR.push_back( sizeA[e] );
    std::vector<std::vector<std::vector<T>>> r(
            sizeR[0], std::vector<std::vector<T>>(
                sizeR[1], std::vector<T>(sizeR[2])
    ));
    std::vector<size_t> i = {0,0,0};
    for ( i[0]=0; i[0]<sizeA[0]; i[0]++ )
        for ( i[1]=0; i[1]<sizeA[1]; i[1]++ )
...
template <class T> std::vector<std::vector<std::vector<std::vector<T>>>>
reshapeOrder(const std::vector<std::vector<std::vector<std::vector<T>>>> & a, const std::vector<size_t> order)
{
    auto & o = order;
    if ( o.size() != 4 ) throw "order must be of size 4";
    std::vector<size_t> sizeA = { a.size(), a[0].size(), a[0][0].size(), a[0][0][0].size() };
    std::vector<size_t> sizeR;
    for ( auto e : o ) sizeR.push_back( sizeA[e] );
    std::vector<std::vector<std::vector<std::vector<T>>>> r(
        sizeR[0], std::vector<std::vector<std::vector<T>>>(
            sizeR[1], std::vector<std::vector<T>>(
                sizeR[2], std::vector<T>(sizeR[3])
    )));
    std::vector<size_t> i = {0,0,0,0};
    for ( i[0]=0; i[0]<sizeA[0]; i[0]++ )
        for ( i[1]=0; i[1]<sizeA[1]; i[1]++ )
            for ( i[2]=0; i[2]<sizeA[2]; i[2]++ )
...
template <class T> std::vector<size_t>
shape(const std::vector<std::vector<T>> & a)
{
    std::vector<size_t> r;
    r.push_back(a.size());
...
template <class T> std::vector<size_t>
shape(const std::vector<std::vector<std::vector<T>>> & a)
{
    std::vector<size_t> r;
    r.push_back(a.size());
    r.push_back(a[0].size());
...
template <class T> std::vector<size_t>
shape(const std::vector<std::vector<std::vector<std::vector<T>>>> & a)
{
    std::vector<size_t> r;
    r.push_back(a.size());
    r.push_back(a[0].size());
...
template <class T> std::vector<std::vector<T>>
square(const std::vector<std::vector<T>> & a)
{
    std::vector<std::vector<T>> r;
    for ( const auto & e : a ) r.push_back( square(e) );
    return r;
...
template <class T> std::vector<std::vector<std::vector<T>>>
square(const std::vector<std::vector<std::vector<T>>> & a)
{
    std::vector<std::vector<std::vector<T>>> r;
    for ( const auto & e : a ) r.push_back( square(e) );
    return r;
...
template <class T> std::vector<std::vector<std::vector<std::vector<T>>>>
square(const std::vector<std::vector<std::vector<std::vector<T>>>> & a)
{
    std::vector<std::vector<std::vector<std::vector<T>>>> r;
    for (const auto & e : a) r.push_back( square(e) );
    return r;
...
template <class T> std::vector<T>
shift(const std::vector<T> & n, size_t s)
{
    std::vector<T> r;
...
template <class T> std::vector<std::vector<T>>
shift(const std::vector<std::vector<T>> & n, size_t s)
{
    std::vector<std::vector<T>> r;
    for ( const auto & e : n ) r.push_back( shift(e, s) );
    return r;
...
template <class T> std::vector<std::vector<std::vector<T>>>
shift(const std::vector<std::vector<std::vector<T>>> & n, size_t s)
{
    std::vector<std::vector<std::vector<T>>> r;
    for ( const auto & e : n ) r.push_back( shift(e, s) );
    return r;
...
template <class T> std::vector<std::vector<std::vector<std::vector<T>>>>
shift(const std::vector<std::vector<std::vector<std::vector<T>>>> & n, size_t s)
{
    std::vector<std::vector<std::vector<std::vector<T>>>> r;
    for ( const auto & e : n ) r.push_back( shift(e, s) );
    return r;
...
template <class T>
std::vector<T> subset(const std::vector<T> & v, size_t begin, size_t end)
{
    std::vector<T> vss;
...
subvector(const std::vector<std::vector<std::vector<std::vector<T>>>> & x, const std::vector<size_t> & indices)
{
    ...
    {
        std::vector<std::vector<std::vector<T>>> n;
